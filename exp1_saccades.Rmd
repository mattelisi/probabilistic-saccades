...
Given a certain variability $\sigma$ of the motor outcomes, and a given cost asymmetry $a$, there is only one ideal aimpoint $\mu$ which minimizes the expected cost. It is straighforward to compute the minimum of the expected cost by gradient descent, using the derivative of the expected loss.

The derivative of the expected loss can be expressed as
$$
\begin{align}
\frac{\rm{d}}{\rm{d}\mu} \mathbb{E}(L \mid \mu ,\sigma)=&(2a-1) \mu \cdot \mathrm{erf}\left( \frac{\mu}{\sqrt{2} \sigma} \right) + \frac{2\sigma \cdot (2a-1) \cdot e^{-\frac{\mu^2}{2 \sigma^2}} }{\sqrt{2\pi}} + \mu \\
=&\mu + (2a-1) \left[ \mu \cdot \mathrm{erf}\left( \frac{\mu}{\sqrt{2} \sigma} \right) + \frac{2\sigma \cdot e^{-\frac{\mu^2}{2 \sigma^2}} }{\sqrt{2\pi}} \right]
\end{align}
$$
Again, if $a = 0.5$ the derivative is zero (indicating the minimum of the function) only when $\mu=0$. 
...
# compute ideal aimpoint using gradient descent
...
idealAimpoint_GD <- function(sig, a){
...
# quadratic model, add another parameter (slope2 * sigma^2)
# par = c(sd1, sd2, sd3, intercept, slope, slope2)
negloglik_m01 <-function(d, par){
	L <- 0
	for(i in 1:3){
		L <- L - sum(dnorm(d$err_g[d$cond==i], mean=par[4]+par[i]*par[5]+par[i]^2*par[6], sd= par[i], log=TRUE))

	}
	return(L)
}

MSE_01 <- function(par, d){
	MSE <- 0
	for(i in 1:3){
		MSE <- MSE + sum(d$err_g[d$cond==i]-(par[4]+par[i]*par[5]+par[i]^2*par[6]))^2
	}
	return(MSE/nrow(d))
}
...
### ------------------------------------------------------------------------------------- ###
# function for estimating also the exponent of the loss
# par = c(exponent, asymmetry)
loss_2 <- function(err, par){
	(par[2] + (1-2*par[2])*(err<0)) * abs(err)^par[1]
}

idealAimpoint_2 <- function(sig, par){
	integrand <-function(x, aim_p) loss_2(x, par)*dnorm(x, mean=aim_p, sd= sig)
	exp_cost <- function(aim_p) integrate(integrand, lower=-Inf, upper=Inf, aim_p=aim_p)$value
	return(optim(0, exp_cost, method = "Brent", lower = -2, upper = 2)$par)
}

idealAimpoint_v_2 <- function(sig, par){
	aimp <- vector("numeric",length(sig))
	for(i in 1:length(sig)){
		aimp[i] <- idealAimpoint_2(sig[i],par)
	}
	return(aimp)
}

# negative log likelihood function for a single subject
# par = c(sig1, sig2, sig3, exponent, alpha)
negloglik_single_2 <- function(par, d){
	aimpoint <- vector("numeric",3)
	L <- 0
	for(i in 1:3){
		integrand <-function(x, aim_p) loss_2(x, par[4:5])*dnorm(x, mean=aim_p, sd= par[i])
		exp_cost <- function(aim_p) integrate(integrand, lower=-Inf, upper=Inf, aim_p=aim_p)$value
		aimpoint[i] <- optim(0, exp_cost, method = "Brent", lower = -2, upper = 2)$par
		L <- L - sum(dnorm(d$err_g[d$cond==i], mean=aimpoint[i], sd= par[i], log=TRUE))
	}
...
MSE_2 <- function(par, d){
	MSE <- 0
	for(i in 1:3){
		MSE <- MSE + sum(d$err_g[d$cond==i]-idealAimpoint_2(par[i], par[4:5]))^2
	}
	return(MSE/nrow(d))
}

### ------------------------------------------------------------------------------------- ###
# alternative version with 2 free parameters in the loss function
# par = c(exponent, asymmetry)
loss_3 <- function(err, par){
	(par[2] + (1-2*par[2])*(err<0)) * (par[1]*err)^2
}

idealAimpoint_3 <- function(sig, par){
	integrand <-function(x, aim_p) loss_3(x, par)*dnorm(x, mean=aim_p, sd= sig)
	exp_cost <- function(aim_p) integrate(integrand, lower=-Inf, upper=Inf, aim_p=aim_p)$value
	return(optim(0, exp_cost, method = "Brent", lower = -2, upper = 2)$par)
}

idealAimpoint_v_3 <- function(sig, par){
	aimp <- vector("numeric",length(sig))
	for(i in 1:length(sig)){
		aimp[i] <- idealAimpoint_3(sig[i],par)
	}
	return(aimp)
}

# negative log likelihood function for a single subject
# par = c(sig1, sig2, sig3, cost_factor, alpha)
negloglik_single_3 <- function(par, d){
	aimpoint <- vector("numeric",3)
	L <- 0
	for(i in 1:3){
		integrand <-function(x, aim_p) loss_3(x, par[4:5])*dnorm(x, mean=aim_p, sd= par[i])
		exp_cost <- function(aim_p) integrate(integrand, lower=-Inf, upper=Inf, aim_p=aim_p)$value
		aimpoint[i] <- optim(0, exp_cost, method = "Brent", lower = -2, upper = 2)$par
		L <- L - sum(dnorm(d$err_g[d$cond==i], mean=aimpoint[i], sd= par[i], log=TRUE))
	}
...
hcoCrossValidation <- function(d){
	...
	for(i in 1:nCs){
		...
		# fit quadratic model
		fit2 <- optim(par=startpar_2, fn=negloglik_single_2, d=dtrain, method="L-BFGS-B", lower=par_LB_2, upper=par_UB_2)
		fit2 <- optim(par=fit2$par, fn=negloglik_single_2, d=dtrain, method="L-BFGS-B", lower=par_LB_2, upper=par_UB_2)
		L2 <- -negloglik_single_2(fit2$par, dtest) # note the minus sign
		MSE2 <- MSE_2(fit2$par, dtest)
		...
		# store data
		dout <- rbind(dout, data.frame(t, c_test, sig1=fit1$par[1], sig2=fit1$par[2], sig3=fit1$par[3], alpha=fit1$par[4], L1, L2, L00, L_R, MSE1, MSE2, MSE00, MSE_R, sig1R=fitR$par[1], sig2R=fitR$par[2], sig3R=fitR$par[3], alphaR=fitR$par[5], lhspR=fitR$par[4],sig1_2=fit2$par[1], sig2_2=fit2$par[2], sig3_2=fit2$par[3], alpha_2=fit2$par[5],expo_2=fit2$par[4]))
