# This script report the analysis of the secondary/corrective saccades
# 
# Matteo Lisi, 2019
...
# load fitted cost function parameters
dfit <- read.table("./data/fit_all_multi.txt",header=T)

# aggregate latency data for plotting
...
# fit multilevel quadratic model
d$rt_sec <- d$rt/1000
d$id2 <- paste(d$id, d$exp,sep="_")
m2all <- lmer(rt_sec ~ ampli + I(ampli^2) + (ampli + I(ampli^2)| id2), d)
summary(m2all)

# plot average latencies (split by direction) with predicted values
# compute predicted latencies as a function of amplitude
...
# run bootstrap to get standard error of predicted latencies
#boot_m2all <- bootMer(m2all, mfun, nsim=500,.progress="txt")
...
# finally do plot
pdf("./Figs/secondary_latency_binned.pdf",width=1.6,height=2.4)
...
## store model residuals for later analysis
...
# custom function to compute boostrapped standard error of latency cost
...
# final scatterplot of cost asymmetry and latency cost
d_a <- d_a[seq(nrow(d_a),1,-1),] # reorder for plotting layers
pdf("./Figs/cost_corr.pdf",width=3.1,height=2.5)
